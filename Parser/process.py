...
def process_html(event, context):
    ...
    for listing in listings:
        ...
        if location_elem:
            location_text = location_elem.get_text(strip=True)  # Ejemplo: "Bogotá, Cedritos"
        ...
        num_habitaciones = bedrooms_elem["content"] if bedrooms_elem and "content" in bedrooms_elem.attrs else "N/A"

        # Extraer número de baños
        bathrooms_elem = listing.find("p", {"data-test": "bathrooms"})
        num_banos = bathrooms_elem["content"] if bathrooms_elem and "content" in bathrooms_elem.attrs else "N/A"

        # Extraer metros cuadrados
        floor_area_elem = listing.find("p", {"data-test": "floor-area"})
        mts2 = floor_area_elem["content"] if floor_area_elem and "content" in floor_area_elem.attrs else "N/A"
