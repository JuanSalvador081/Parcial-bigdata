...
import datetime
from Scrapper import scrap  # Se asume que la función scrape_and_upload está en el módulo scrap

# Prueba 1: Verifica que, ante respuestas HTTP 200, se procese y se llame a s3.put_object
...
def test_scrape_success(mock_put_object, mock_requests_get):
    ...
    # Verificamos que se haya llamado a s3.put_object y se retorne el mensaje esperado
    mock_put_object.assert_called_once()
    assert "Scraping completado" in result["message"]

# Prueba 2: Simula que algunas páginas fallan (por ejemplo, status 404) y verifica que se complete el proceso
@patch('Scrapper.scrap.requests.get')
@patch('Scrapper.scrap.s3.put_object')
def test_scrape_partial_failure(mock_put_object, mock_requests_get):
    def side_effect(*args, **kwargs):
        ...
        response = MagicMock()
        # Si el número de página es par, simula error 404, de lo contrario éxito
        if page % 2 == 0:
    ...
    result = scrap.scrape_and_upload({}, {})
    # A pesar de errores intermitentes, se espera que se invoque s3.put_object y se retorne mensaje de éxito
...
def test_filename_format():
    ...
    # Se espera que el formato sea "YYYY-MM-DD.html" (longitud fija para una fecha válida)
